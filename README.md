...
| `admin` | 系统管理员 | 组织与成员、能力模型、工位极限、服务身份、指标定义 |
...
| 治理 | 指标定义 / 审计记录 / 系统治理 | 账号创建、角色/成员状态、首次改密与口令重置；服务身份签发、授权编辑、密钥轮换、停用；拒绝访问日志 |
...
| 心跳超期或安全联锁 | 执行门关闭，排程、下发与续跑返回 423；队列里的动作指令不投递 |
...
api/alembic/ 版本化迁移：0001 基线 → 0002 结构 → 0003 历史映射 → 0004 适配器配置 → 0005 样本关联 → 0006 服务身份并发版本 → 0007 账号生命周期 → 0008 推进事件重试计数 → 0009 运行加固
